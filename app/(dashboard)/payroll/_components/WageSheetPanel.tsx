...
export default function WageSheetPanel({ onClose, onDone }: { onClose: () => void; onDone?: (next?: string) => void }) {
    ...
    useEffect(()=>{ fetch("/api/sites?isActive=true").then(r=>r.json()).then(d=>{if(Array.isArray(d))setSites(d)}).finally(()=>setLdSites(false)) },[])

    const fetchStatus = useCallback(async ()=>{
        try { const r=await fetch(`/api/payroll/sites-status?month=${month}&year=${year}`); if(r.ok){const d=await r.json();if(Array.isArray(d))setStatus(d)} } catch {}
    },[month,year])
    ...
    const selectSite = (id: string) => { setSelId(id); setSearch(""); setSelectedIds(new Set()); if(id) fetchData(id); else setData([]) }
    const toggleSelect = (id: string) => setSelectedIds(prev=>{ const s=new Set(prev); s.has(id)?s.delete(id):s.add(id); return s })
    const toggleAll = (rows: Payroll[]) => setSelectedIds(prev=>prev.size===rows.length?new Set():new Set(rows.map(p=>p.id)))

    const buildFormIIAoa = (rows: Payroll[], siteName: string, m: number, y: number) => {
        const monthName=MONTHS[m-1].toUpperCase()
        const headers=["Sr.No","Emp Code","Employee Name","Bank A/C No.","IFSC","UAN","PF No.","ESIC No.","Month Days","LOP","Days Paid","OT Days","OT Hrs","Basic(R)","DA(R)","HRA(R)","Washing(R)","Conv.(R)","LWW(R)","Bonus(R)","Other(R)","Gross(R)","Basic","DA","HRA","Washing","Conv.","LWW","Bonus","Other","OT Amt","Prod.Inc","Gross Earning","PF(Emp)","ESIC(Emp)","PT","LWF","Canteen","Other Ded.","Advance","Penalty","Tot. Ded.","Net Pay","PF(Er)","ESIC(Er)","CTC","Signature"]
        const dataRows=rows.map((p,i)=>{
            const monthDays=p.workingDays??26; const workedDays=p.presentDays??monthDays
            const lop=monthDays-workedDays; const otDays=p.otDays??0; const otHrs=p.overtimeHrs??Math.round(otDays*8)
            return [i+1,p.employee.employeeId,`${p.employee.firstName} ${p.employee.lastName}`.trim(),p.employee.bankAccountNumber??"",p.employee.bankIFSC??"",p.employee.uan??"",p.employee.pfNumber??"",p.employee.esiNumber??"",monthDays,lop,workedDays,otDays,otHrs,Math.round(p.basicFull??p.basicSalary),Math.round(p.daFull??p.da),Math.round(p.hraFull??p.hra),Math.round(p.washingFull??p.washing),Math.round(p.conveyanceFull??p.conveyance),Math.round(p.lwwFull??p.lwwEarned),Math.round(p.bonusFull??p.bonus),Math.round(p.otherFull),Math.round(p.grossFullMonth),Math.round(p.basicSalary),Math.round(p.da),Math.round(p.hra),Math.round(p.washing),Math.round(p.conveyance),Math.round(p.lwwEarned),Math.round(p.bonus),Math.round(p.allowances),Math.round(p.overtimePay),Math.round(p.productionIncentive),Math.round(p.grossSalary),Math.round(p.pfEmployee),Math.round(p.esiEmployee),Math.round(p.pt),Math.round(p.lwf),Math.round(p.canteen),Math.round(p.otherDeductions),Math.round(p.advance),Math.round(p.penalty),Math.round(p.totalDeductions),Math.round(p.netSalary),Math.round(p.pfEmployer),Math.round(p.esiEmployer),Math.round(p.ctc),""]
        })
        const sum=(fn:(p:Payroll)=>number)=>Math.round(rows.reduce((s,p)=>s+fn(p),0))
        const totRow=["","","TOTAL","","","","","","","",sum(p=>p.presentDays??p.workingDays??0),"","",sum(p=>p.basicFull??p.basicSalary),sum(p=>p.daFull??p.da),sum(p=>p.hraFull??p.hra),sum(p=>p.washingFull??p.washing),sum(p=>p.conveyanceFull??p.conveyance),sum(p=>p.lwwFull??p.lwwEarned),sum(p=>p.bonusFull??p.bonus),sum(p=>p.otherFull??0),sum(p=>p.grossFullMonth),sum(p=>p.basicSalary),sum(p=>p.da),sum(p=>p.hra),sum(p=>p.washing),sum(p=>p.conveyance),sum(p=>p.lwwEarned),sum(p=>p.bonus),sum(p=>p.allowances),sum(p=>p.overtimePay),sum(p=>p.productionIncentive),sum(p=>p.grossSalary),sum(p=>p.pfEmployee),sum(p=>p.esiEmployee),sum(p=>p.pt),sum(p=>p.lwf),sum(p=>p.canteen),sum(p=>p.otherDeductions),sum(p=>p.advance),sum(p=>p.penalty),sum(p=>p.totalDeductions),sum(p=>p.netSalary),sum(p=>p.pfEmployer),sum(p=>p.esiEmployer),sum(p=>p.ctc),""]
        return [[`FORM (II) M.W. RULES Rule (27)(1)`],[`SALARIES / WAGES REGISTER FOR THE MONTH OF ${monthName} ${y}`],[`SITE: ${siteName}`],headers,...dataRows,totRow]
    }
    ...
    const handleExportCombined = async ()=>{
        ...
        try {
            ...
            for (const siteId of selectedSiteIds) {
                ...
                const r=await fetch(`/api/payroll?siteId=${siteId}&month=${month}&year=${year}`)
                if (!r.ok) continue; const rows: Payroll[]=await r.json(); if (!rows.length) continue
                const aoa=buildFormIIAoa(rows,site?.name??siteId,m,y)
                const ws=XLSX.utils.aoa_to_sheet(aoa)
                ws["!cols"]=[5,10,24,14,12,12,10,10,8,5,8,7,6,8,7,7,9,7,7,8,7,9,8,7,7,8,7,7,8,7,8,8,11,8,9,6,6,8,9,8,8,9,9,7,8,10,12].map(w=>({wch:w}))
                const sheetName=(site?.name??siteId).replace(/[^a-zA-Z0-9 ]/g,"").trim().slice(0,31)||`Site${sheetsAdded+1}`
                XLSX.utils.book_append_sheet(wb,ws,sheetName); sheetsAdded++
            }
    ...
    const handleNEFT = ()=>{
        ...
        const rows=exportRows.map(p=>["N","",p.employee.bankAccountNumber||"",Math.round(p.netSalary),`${p.employee.firstName} ${p.employee.lastName}`.toUpperCase(),"","","","","","","",caption,caption,siteShort,"","","","","","","",payDate,"",p.employee.bankIFSC||"","","",neftEmail])
    ...
    const handleStatementExcel = ()=>{
        ...
        const aoa: unknown[][] = [["","","File Transactions"],[],[],["",hdfcGenBy||"ADMIN","Generated By   :   ",site?.name??""],["",`Generated On   :   `,genOn],[],[],["","Account Number","Beneficiary","Beneficiary Account No.","IFSC Code","User Reference Number","Value date","Transfer Type","Amount","Status"],...exportRows.map(p=>["",hdfcAccNo||"XXXXXXXXXXXXXXXXX",`${p.employee.firstName} ${p.employee.lastName}`.toUpperCase(),p.employee.bankAccountNumber||"",p.employee.bankIFSC||"",caption,valueDate,"NEFT",Math.round(p.netSalary),"Processed"]),[],[``,`Total Search Results  :  `,exportRows.length]]
    ...
    const handleStatementPDF = ()=>{
        ...
        const site=sites.find(s=>s.id===selId); const m=parseInt(month); const y=parseInt(year)
        const caption=`Salary ${MONTHS[m-1].slice(0,3)} ${String(y).slice(-2)}`
        const today=new Date(); const genOn=today.toLocaleString("en-IN"); const valueDate=today.toLocaleDateString("en-IN")
        const totalAmt=exportRows.reduce((s,p)=>s+Math.round(p.netSalary),0)
        const rows=exportRows.map((p,i)=>`<tr style="background:${i%2===0?"#fff":"#f8fafc"}"><td>${i+1}</td><td>${hdfcAccNo||"XXXXXXXXXXXXXXXXX"}</td><td style="text-align:left;font-weight:600">${`${p.employee.firstName} ${p.employee.lastName}`.toUpperCase()}</td><td>${p.employee.bankAccountNumber||"<span style='color:#dc2626'>Missing</span>"}</td><td>${p.employee.bankIFSC||"<span style='color:#dc2626'>Missing</span>"}</td><td>${caption}</td><td>${valueDate}</td><td>NEFT</td><td style="text-align:right;font-weight:700">₹${Math.round(p.netSalary).toLocaleString("en-IN")}</td><td style="color:#16a34a;font-weight:600">Processed</td></tr>`).join("")
        const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>HDFC Bank Statement</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:11px;color:#1a1a1a;padding:20px}.header{display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #004C8F;padding-bottom:12px;margin-bottom:16px}.logo-text{font-size:26px;font-weight:900;color:#004C8F}.logo-sub{font-size:9px;color:#004C8F;letter-spacing:1px}.logo-tag{font-size:8px;color:#e4002b;font-style:italic}.bank-info{text-align:right;font-size:10px;color:#555}.title-bar{background:#004C8F;color:#fff;padding:8px 14px;border-radius:4px;margin-bottom:14px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:6px 20px;background:#f0f4f8;border:1px solid #dce5ef;border-radius:4px;padding:10px 14px;margin-bottom:14px;font-size:10px}.meta span{color:#555}.meta b{color:#004C8F}table{width:100%;border-collapse:collapse;font-size:10px}th{background:#004C8F;color:#fff;padding:7px 8px;text-align:center;white-space:nowrap;font-weight:700}td{padding:5px 8px;border-bottom:1px solid #e5e7eb;text-align:center;white-space:nowrap}.footer{margin-top:14px;display:flex;justify-content:space-between;align-items:center;border-top:2px solid #004C8F;padding-top:10px}.footer-total{background:#004C8F;color:#fff;padding:6px 16px;border-radius:4px;font-weight:700;font-size:12px}.footer-note{font-size:9px;color:#888}@media print{body{padding:10px}}</style></head><body><div class="header"><div><div class="logo-text">HDFC BANK</div><div class="logo-sub">WE UNDERSTAND YOUR WORLD</div><div class="logo-tag">India's Most Trusted Private Bank</div></div><div class="bank-info"><div><b>Statement Type:</b> File Transactions</div><div><b>Site:</b> ${site?.name??"—"}</div><div><b>Period:</b> ${MONTHS[m-1]} ${y}</div></div></div><div class="title-bar"><h2>File Transactions — Salary Payment Statement</h2></div><div class="meta"><span><b>User ID</b>: ${hdfcGenBy||"ADMIN"}</span><span><b>Generated On</b>: ${genOn}</span><span><b>Company Account</b>: ${hdfcAccNo||"XXXXXXXXXXXXXXXXX"}</span><span><b>Transfer Type</b>: NEFT</span><span><b>Value Date</b>: ${valueDate}</span><span><b>Reference</b>: ${caption}</span></div><table><thead><tr><th>#</th><th>Account Number</th><th>Beneficiary</th><th>Beneficiary A/C No.</th><th>IFSC Code</th><th>User Reference</th><th>Value Date</th><th>Transfer Type</th><th>Amount (₹)</th><th>Status</th></tr></thead><tbody>${rows}</tbody><tfoot><tr style="background:#eff6ff;font-weight:700"><td colspan="8" style="text-align:right;color:#004C8F">Total (${exportRows.length} records)</td><td style="text-align:right;color:#004C8F">₹${totalAmt.toLocaleString("en-IN")}</td><td></td></tr></tfoot></table><div class="footer"><div class="footer-note">Computer generated statement. No signature required.</div><div class="footer-total">Total Payable: ₹${totalAmt.toLocaleString("en-IN")}</div></div></body></html>`
        printHTML(html)
    }
    ...
    const handleExport = ()=>{
        ...
        const wb=XLSX.utils.book_new(); const ws=XLSX.utils.aoa_to_sheet(aoa)
        ws["!cols"]=[6,12,26,14,18,10,8,10,8,8,10,10,10,10,4,10,8,8,14,12,10,10,10,14,8,8,6,6,8,10,10,12,10,12].map(w=>({wch:w}))
    ...
    const handleLock = async ()=>{
        ...
        setLocking(true)
        try {
            const res=await fetch("/api/payroll/final/lock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({month:parseInt(month),year:parseInt(year),payrollIds:[...selectedIds]})})
            if (!res.ok) throw new Error(await res.text())
            const d=await res.json()
            toast.success(`${d.count} employee(s) locked — going to Compliance`)
            setSelectedIds(new Set()); await fetchData(selId)
            onDone?.("compliance")
    ...
    const handleLockSites = async ()=>{
        ...
        try {
            const res=await fetch("/api/payroll/final/lock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({month:parseInt(month),year:parseInt(year),siteIds:[...selectedSiteIds]})})
            if (!res.ok) throw new Error(await res.text())
            const d=await res.json()
            toast.success(`${d.count} employee(s) locked from ${selectedSiteIds.size} site(s)`)
            setSelectedSiteIds(new Set()); if(selId) await fetchData(selId); await fetchStatus()
            onDone?.("compliance")
        } catch(e: unknown) { toast.error(e instanceof Error?e.message:"Lock failed") }
    ...
    const filtered       = data.filter(p=>!search||`${p.employee.firstName} ${p.employee.lastName} ${p.employee.employeeId}`.toLowerCase().includes(search.toLowerCase()))
    ...
    const totals = { gross:data.reduce((s,p)=>s+p.grossSalary,0), net:data.reduce((s,p)=>s+p.netSalary,0), ded:data.reduce((s,p)=>s+p.totalDeductions,0), pf:data.reduce((s,p)=>s+p.pfEmployee,0), esi:data.reduce((s,p)=>s+p.esiEmployee,0) }
    ...
    return (
        <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:32 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 20px", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12 }}>
                ...
                <button onClick={onClose} style={{ width:32, height:32, borderRadius:8, border:"1px solid var(--border)", background:"var(--surface2)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", color:"var(--text3)" }}>
                    <X size={16} />
                </button>
            </div>
                ...
                <div style={{ width:256, flexShrink:0, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, overflow:"hidden", display:"flex", flexDirection:"column" }}>
                    ...
                    <div style={{ overflowY:"auto", flex:1, maxHeight:460 }}>
                        {ldSites ? <div style={{ padding:24, textAlign:"center" }}><Loader2 size={16} className="animate-spin" style={{ color:"var(--accent)", margin:"0 auto" }} /></div> : (
                            <>
                                <div onClick={()=>selectSite("")} style={{ padding:"10px 14px", cursor:"pointer", borderBottom:"1px solid var(--border)", background:!selId?"var(--accent-light)":"transparent", borderLeft:!selId?"3px solid var(--accent)":"3px solid transparent" }}>
                                ...
                                {processedSites.map(site=>{
                                    ...
                                    return (
                                        <div key={site.id} onClick={()=>selectSite(site.id)}
                                            style={{ padding:"10px 14px", cursor:"pointer", borderBottom:"1px solid var(--border)", background:isSiteChecked?"#f5f3ff":isSel?"var(--accent-light)":"transparent", borderLeft:isSiteChecked?"3px solid #7c3aed":isSel?"3px solid var(--accent)":"3px solid transparent" }}>
                                            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:6 }}>
                                                <div style={{ display:"flex", alignItems:"center", gap:6, minWidth:0 }}>
                                                    <input type="checkbox" checked={isSiteChecked} onClick={e=>toggleSiteSelect(site.id,e)} onChange={()=>{}} style={{ cursor:"pointer", accentColor:"#7c3aed", flexShrink:0 }} />
                                                ...
                                                <span style={{ padding:"1px 6px", borderRadius:20, fontSize:9, fontWeight:700, whiteSpace:"nowrap", background:isDone?"#dcfce7":"#fef9c3", color:isDone?"#15803d":"#854d0e" }}>{isDone?`${st!.processedCount}`:"—"}</span>
                                            </div>
                    ...
                    {selectedSiteIds.size>0 && (
                        <div style={{ padding:"10px 14px", borderTop:"2px solid #7c3aed", background:"#f5f3ff" }}>
                            <div style={{ fontSize:11, color:"#7c3aed", fontWeight:700, marginBottom:8 }}>{selectedSiteIds.size} site{selectedSiteIds.size>1?"s":""} selected</div>
                            <button onClick={handleExportCombined} disabled={dlCombined}
                                style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"7px 12px", borderRadius:8, border:"1px solid #7c3aed", background:"#fff", color:"#7c3aed", fontSize:12, fontWeight:700, cursor:dlCombined?"not-allowed":"pointer", marginBottom:6 }}>
                            ...
                            {processedSites.length===0 ? (
                                ...
                                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))", gap:10 }}>
                                    {processedSites.map(site=>{
                                        ...
                                        return (
                                            <div key={site.id} onClick={()=>selectSite(site.id)}
                                                style={{ padding:"14px 16px", borderRadius:10, cursor:"pointer", border:`1px solid ${isDone?"#86efac":"var(--border)"}`, background:isDone?"#f0fdf4":"var(--surface)" }}
                                                ...
                                                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:6 }}>
                                                    ...
                                                    <span style={{ padding:"2px 7px", borderRadius:20, fontSize:9, fontWeight:700, background:isDone?"#dcfce7":"#fef9c3", color:isDone?"#15803d":"#854d0e", whiteSpace:"nowrap" }}>{isDone?"✓ Done":"No Data"}</span>
                                                </div>
                                                <div style={{ marginTop:10, fontSize:11, color:isDone?"#15803d":"var(--text3)", fontWeight:isDone?600:400 }}>{isDone?`${st!.processedCount} employees`:"Not yet processed"}</div>
                                            </div>
                                        )
                                    })}
                                </div>
                        ...
                        <>
                            ...
                            <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, padding:"11px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap" }}>
                                ...
                                <div style={{ display:"flex", gap:8 }}>
                                    ...
                                    {activeView==="wagesheet" && (
                                        <button onClick={handleExport} disabled={!data.length} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"none", background:"#16a34a", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", opacity:!data.length?0.5:1 }}>
                                            <FileSpreadsheet size={13} />{selectedCount>0?`Form II (${selectedCount} selected)`:"Form II Excel"}
                                        </button>
                                    )}
                                    {activeView==="neft" && (
                                        <button onClick={handleNEFT} disabled={!data.length} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"none", background:"#0369a1", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", opacity:!data.length?0.5:1 }}>
                                            <FileDown size={13} />{selectedCount>0?`Download NEFT (${selectedCount})`:"Download NEFT"}
                                        </button>
                                    )}
                                    {activeView==="statement" && (
                                        <>
                                            <button onClick={handleStatementExcel} disabled={!data.length} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"none", background:"#16a34a", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", opacity:!data.length?0.5:1 }}>
                                                <FileSpreadsheet size={13}/> Excel
                                            </button>
                                            <button onClick={handleStatementPDF} disabled={!data.length} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"none", background:"#dc2626", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", opacity:!data.length?0.5:1 }}>
                            ...
                            {activeView==="statement" && (
                                <div style={{ background:"#fff7ed", border:"1px solid #fed7aa", borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                                    ...
                                    <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                                        ...
                                        <input value={hdfcAccNo} onChange={e=>setHdfcAccNo(e.target.value)} placeholder="e.g. 50200061320551"
                                    ...
                                    <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                                        ...
                                        <input value={hdfcGenBy} onChange={e=>setHdfcGenBy(e.target.value)} placeholder="e.g. GROAUCX"
                                    ...
                                    <span style={{ fontSize:11, color:"#ea580c", marginLeft:"auto" }}>PDF opens print dialog · Excel downloads instantly</span>
                                </div>
                            )}
                            ...
                            {activeView==="neft" && (
                                <div style={{ background:"#eff6ff", border:"1px solid #bfdbfe", borderRadius:10, padding:"12px 16px", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                                    ...
                                    <input value={neftEmail} onChange={e=>setNeftEmail(e.target.value)} placeholder="accounts@yourcompany.com"
                                    ...
                                    <span style={{ fontSize:11, color:"#3b82f6" }}>Site short: <b>{(selSite?.code||selSite?.name||"").toUpperCase().replace(/\s+/g,"").slice(0,8)}</b></span>
                                </div>
                            )}
                                        ...
                                        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                                            ...
                                            <tbody>
                                                {data.slice(0,5).map((p,i)=>{
                                                    ...
                                                    const m=parseInt(month); const caption=`Salary ${MONTHS[m-1].slice(0,3)} ${String(parseInt(year)).slice(-2)}`
                                                    const today=new Date(); const payDate=`${String(today.getDate()).padStart(2,"0")}/${String(today.getMonth()+1).padStart(2,"0")}/${today.getFullYear()}`
                                                    return (
                                                ...
                                                {data.length>5 && <tr><td colSpan={16} style={{ padding:"8px 14px", textAlign:"center", fontSize:11, color:"var(--text3)", background:"#f0f9ff" }}>+ {data.length-5} more rows in downloaded file</td></tr>}
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            ...
                            {activeView==="wagesheet" && (
                                <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, overflow:"hidden" }}>
                                    <div style={{ padding:"9px 14px", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"center", gap:8 }}>
                                        ...
                                        {selectedCount>0 && <span style={{ fontSize:11, fontWeight:700, color:"var(--accent)", whiteSpace:"nowrap", background:"var(--accent-light)", padding:"2px 8px", borderRadius:20 }}>{selectedCount} selected</span>}
                                        ...
                                        <table style={{ width:"max-content", minWidth:"100%", borderCollapse:"collapse", fontSize:11 }}>
                                            <thead>
                                                ...
                                                <tr style={{ background:"var(--surface2)", borderBottom:"2px solid var(--border)" }}>
                                                    <th style={th}><input type="checkbox" checked={filtered.length>0&&filtered.every(p=>selectedIds.has(p.id))} onChange={()=>toggleAll(filtered)} style={{ cursor:"pointer", accentColor:"var(--accent)" }} /></th>
                                                    <th style={th}>SR</th><th style={th}>EMP CODE</th><th style={{...th,textAlign:"left"}}>NAME</th><th style={th}>DESIG.</th>
                                                    <th style={{...th,background:"#eff6ff"}}>Month<br/>Days</th><th style={{...th,background:"#eff6ff"}}>LOP</th><th style={{...th,background:"#eff6ff"}}>DAYS</th><th style={{...th,background:"#eff6ff"}}>OT<br/>DAYS</th><th style={{...th,background:"#eff6ff"}}>OT<br/>Hrs</th>
                                                    <th style={{...th,background:"#f0fdf4"}}>BASIC</th><th style={{...th,background:"#f0fdf4"}}>DA</th><th style={{...th,background:"#f0fdf4"}}>HRA</th><th style={{...th,background:"#f0fdf4"}}>Washing</th><th style={{...th,background:"#f0fdf4"}}>Convence</th><th style={{...th,background:"#f0fdf4"}}>LWW</th><th style={{...th,background:"#f0fdf4"}}>BONUS</th><th style={{...th,background:"#f0fdf4"}}>Other</th><th style={{...th,background:"#dcfce7",color:"#15803d"}}>GROSS</th>
                                                    <th style={{...th,background:"#fffbeb"}}>BASIC</th><th style={{...th,background:"#fffbeb"}}>DA</th><th style={{...th,background:"#fffbeb"}}>HRA</th><th style={{...th,background:"#fffbeb"}}>Washing</th><th style={{...th,background:"#fffbeb"}}>Convence</th><th style={{...th,background:"#fffbeb"}}>LWW</th><th style={{...th,background:"#fffbeb"}}>Bonus</th><th style={{...th,background:"#fffbeb"}}>Other</th><th style={{...th,background:"#fffbeb"}}>OT</th><th style={{...th,background:"#fffbeb"}}>Prod<br/>Incentive</th><th style={{...th,background:"#fef9c3",color:"#854d0e"}}>GROSS</th>
                                                    <th style={{...th,background:"#fef2f2"}}>PF</th><th style={{...th,background:"#fef2f2"}}>ESIC</th><th style={{...th,background:"#fef2f2"}}>PT</th><th style={{...th,background:"#fef2f2"}}>LWF</th><th style={{...th,background:"#fef2f2"}}>CANTEEN</th><th style={{...th,background:"#fef2f2"}}>PENALTY</th><th style={{...th,background:"#fef2f2"}}>ADVANCE</th><th style={{...th,background:"#fef2f2"}}>Other<br/>Ded</th><th style={{...th,background:"#fee2e2",color:"#b91c1c"}}>Total<br/>Deduction</th>
                                                    <th style={{...th,background:"#dbeafe"}}>CO CONTRI<br/>PF</th><th style={{...th,background:"#dbeafe"}}>CO CONTRI<br/>ESIC</th><th style={{...th,background:"#bfdbfe",color:"#1e40af"}}>CTC</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {ldData ? (
                                                    <tr><td colSpan={43} style={{ padding:"40px 0", textAlign:"center" }}><Loader2 size={20} className="animate-spin" style={{ color:"var(--accent)", margin:"0 auto" }} /></td></tr>
                                                ) : filtered.length===0 ? (
                                                    <tr><td colSpan={43} style={{ padding:"30px", textAlign:"center", color:"var(--text3)", fontSize:12 }}>{data.length===0?"No processed payroll records found":"No results match your search"}</td></tr>
                                                ) : filtered.map((p,i)=>{
                                                    const monthDays=p.workingDays??26; const workedDays=p.presentDays??monthDays; const lop=monthDays-workedDays
                                                    const otHrs=p.overtimeHrs??Math.round((p.otDays??0)*8)
                                                    return (
                                                        <tr key={p.id} onClick={()=>toggleSelect(p.id)} style={{ borderBottom:"1px solid var(--border)", cursor:"pointer", background:selectedIds.has(p.id)?"var(--accent-light)":i%2===0?"var(--surface)":"var(--surface2)" }}>
                                                            <td style={td} onClick={e=>e.stopPropagation()}><input type="checkbox" checked={selectedIds.has(p.id)} onChange={()=>toggleSelect(p.id)} style={{ cursor:"pointer", accentColor:"var(--accent)" }} /></td>
                                                            <td style={td}>{i+1}</td>
