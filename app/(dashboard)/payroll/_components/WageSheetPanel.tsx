...
export default function WageSheetPanel({ onClose, onDone }: { onClose: () => void; onDone?: (next?: string) => void }) {
    ...
    const buildFormIIAoa = (rows: Payroll[], siteName: string, m: number, y: number) => {
        ...
        const totRow=["","","TOTAL","","","","","","","",sum(p=>p.presentDays??p.workingDays??0),"","",sum(p=>p.basicFull??p.basicSalary),sum(p=>p.daFull??p.da),sum(p=>p.hraFull??p.hra),sum(p=>p.washingFull??p.washing),sum(p=>p.conveyanceFull??p.conveyance),sum(p=>p.lwwFull??p.lwwEarned),sum(p=>p.bonusFull),sum(p=>p.otherFull),sum(p=>p.grossFullMonth),sum(p=>p.basicSalary),sum(p=>p.da),sum(p=>p.hra),sum(p=>p.washing),sum(p=>p.conveyance),sum(p=>p.lwwEarned),sum(p=>p.bonus),sum(p=>p.allowances),sum(p=>p.overtimePay),sum(p=>p.productionIncentive),sum(p=>p.grossSalary),sum(p=>p.pfEmployee),sum(p=>p.esiEmployee),sum(p=>p.pt),sum(p=>p.lwf),sum(p=>p.canteen),sum(p=>p.otherDeductions),sum(p=>p.advance),sum(p=>p.penalty),sum(p=>p.totalDeductions),sum(p=>p.netSalary),sum(p=>p.pfEmployer),sum(p=>p.esiEmployer),sum(p=>p.ctc),""]
