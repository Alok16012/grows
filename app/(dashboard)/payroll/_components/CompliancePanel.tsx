...
const WAGE_ITEMS: ReportItem[] = [{ id:"wage-sheet", label:"Wage Sheet — Form II (MW Rules)", shortLabel:"Wage Sheet", type:"wage-sheet" }]
...
async function buildHierarchicalSheet(data: Record<string,string|number>[], reportType: string, title: string, m: number, y: number) {
    ...
    let cursor=0
    for (const [,span] of sections) { if(span>1) merges.push({s:{r:2,c:cursor},e:{r:2,c:cursor+span-1}}); cursor+=span }
    ws["!merges"]=merges
    ws["!cols"]=cols.map(k=>({ wch:Math.min(Math.max(Math.max(k.length,...data.map(r=>String(r[k]??"").length))+2,12),30) }))
    ws["!rows"]=[{hpt:24},{hpt:16},{hpt:20},{hpt:22}]
    return ws
}

const COLS = [
    { key:"wage", title:"Wage Sheet",       color:"#0f766e", items:WAGE_ITEMS },
    { key:"pf",   title:"Provident Fund",   color:"#1d4ed8", items:PF_ITEMS   },
    { key:"esic", title:"ESIC",             color:"#0369a1", items:ESIC_ITEMS },
    { key:"pt",   title:"Professional Tax", color:"#7c3aed", items:PT_ITEMS   },
]

function PillCell({ items, run, color, dlLoading, onDownload }: {
    items: ReportItem[]; run: PayrollRun; color: string
    dlLoading: string|null; onDownload: (item: ReportItem, month: number, year: number) => void
}) {
    return (
        <div style={{ padding:"10px 12px", borderLeft:"1px solid var(--border)", display:"flex", flexWrap:"wrap", gap:6, alignContent:"center", alignItems:"center" }}>
            {items.map(item=>{
                const key=`${item.id}-${run.month}-${run.year}`; const isLoading=dlLoading===key
                return (
                    <button key={item.id} onClick={()=>onDownload(item,run.month,run.year)} disabled={isLoading} title={item.label}
                        style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"4px 10px", borderRadius:20, border:`1px solid ${color}22`, background:`${color}11`, color, fontSize:11, fontWeight:600, cursor:isLoading?"not-allowed":"pointer", opacity:isLoading?0.6:1, whiteSpace:"nowrap", transition:"all 0.15s" }}
                        onMouseEnter={e=>{if(!isLoading){(e.currentTarget as HTMLButtonElement).style.background=`${color}22`;(e.currentTarget as HTMLButtonElement).style.borderColor=`${color}55`}}}
                        onMouseLeave={e=>{(e.currentTarget as HTMLButtonElement).style.background=`${color}11`;(e.currentTarget as HTMLButtonElement).style.borderColor=`${color}22`}}>
...
const thStyle: React.CSSProperties = { padding:"12px 16px", fontSize:11, fontWeight:700, color:"var(--text3)", textTransform:"uppercase", letterSpacing:"0.5px", display:"flex", alignItems:"center", gap:6 }

export default function CompliancePanel({ onClose }: { onClose: () => void }) {
    ...
    const fetchRuns = async () => {
        ...
        } catch { toast.error("Failed to load payroll months") }
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16, paddingBottom:40 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap", padding:"14px 20px", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <div>
                        <p style={{ fontSize:16, fontWeight:800, color:"var(--text)", margin:0 }}>Compliance Reports</p>
                ...
                <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                    ...
                    <button
                        onClick={()=>window.open("/payroll/compliance/master","_blank")}
                ...
                <div style={{ display:"grid", gridTemplateColumns:"160px 1fr 2fr 1.5fr 1.5fr", minWidth:900, borderBottom:"2px solid var(--border)", background:"var(--surface2)" }}>
                    ...
                    {COLS.map(col=>(
                        <div key={col.key} style={{...thStyle, color:col.color, borderLeft:"1px solid var(--border)"}}>
                            <FileSpreadsheet size={13} style={{ flexShrink:0 }} />{col.title}
                        </div>
                    ))}
                </div>
                {loading ? (
            ...
            {!loading && runs.length>0 && (
                <p style={{ fontSize:11, color:"var(--text3)", textAlign:"center" }}>Click any pill to instantly download that report. Latest month shown first.</p>
            )}
        </div>
    )
}
